...
hypertrophic cardiomyopathies (HCM).**The raw files containing the detected circRNAs
are not part of circRNAprofiler package. After the circRNA detections we locally 
setup the project folder as described in Module 1 and ran through Modules 2 and 3
to generate the data object backSplicedJunctions and mergedBSJunctions which can
be loaded and used for example purpose in the package vignettes.
The user should setup its own project folder as specified by Module 1 and then
import the detected circRNAs using Module 2.** 
...
```{r, echo=FALSE, out.width='45%', fig.align="center", fig.cap="\\label{fig:figs} Figure 2: Example of a project folder structure"}
...
**Next, set project folder projectFolderName (in this case circRNAprofiler) 
...
subfolder "other".  In this last case, you must ensure that each circRNAs_X.txt
file must have at least the following 6 columns with the header (tab separated):
...
```{r}
# For example purpose we put "experiment.txt" in "extdata" so that it can be used 
# with this vignettes
...
# For example purpose load a short version of the formatted gencode.V19.annotation.gtf 
# which was downloaded from https://www.gencodegenes.org/.
# We ran the above command with pathToGTF = "gencode.V19.annotation.gtf" 
# to create the object "gtf"
data("gtf")
head(gtf)
...
```{r}
# Setup your project folder as described in Module 1.
# Type ?getBackSplicedJunctions for more information about the arguments of the 
# function and its example.
# backSplicedJunctions <- getBackSplicedJunctions(gtf, pathToExperiment)

# For example purpose we load the object resulting by running the above command
# in the project folder that was setup locally which contained the raw files with
...
# For example purpose we load the object resulting by running the above command
# in the project folder that was setup locally.
...
filteredCirc <-
filterCirc(mergedBSJunctions, allSamples = FALSE, min = 5, pathToExperiment)
```
...
flanking introns. The genomic features are exracted from the user provided gene
...
```{r}
# Type ?annotateBSJs for more information about the arguments of the function 
# and its example.

# If you want to analysis specific transcripts report them in transcripts.txt (optional).
# For example purpose we put "transcripts.txt" in "extdata" so that it can be used 
# with this vignettes
pathToTranscripts <- system.file("extdata", "transcripts.txt", package="circRNAprofiler")

# As an example of the 1458 filtered circRNAs we annotate only the firt 30 
# circRNAs
annotatedBSJs <- annotateBSJs(filteredCirc[1:30,], gtf, isRandom = FALSE, pathToTranscripts) 
head(annotatedBSJs)
```
...
```{r}
# Type ?getRandomBSJunctions for more information about the arguments of the 
# function and its example.
...
# Type ?plotLenIntrons for more information about the arguments of the function 
# and its example.
...
# Type ?plotLenBSEs for more information about the arguments of the function and 
# its example.
p2 <- plotLenBSEs(
    annotatedBSJs,
    annotatedRBSJs,
...
# Type ?plotHostGenes for more information about the arguments of the function
# and its example.
p3 <-
    plotHostGenes(annotatedBSJs, title = "# CircRNAs produced from host genes")

# No. of exons in between the back-spliced junctions
# Type ?plotExBetweenBSEs for more information about the arguments of the function
# and its example.
p4 <-
...
# Type ?plotExPosition for more information about the arguments of the function
# and its example.
p5 <-
...
# Type ?plotTotExons for more information about the arguments of the function
# and its example.
p6 <-
...
A motif identified in target sequences is selected if it matches with or they 
are part (as substring) of any RBP motifs deposited in the database. It is possible 
...
# For example purpose we put "motifs.txt" in "extdata" so that it can be used 
# with this vignettes
# E.g. in motifs.txt we added RBM20 consensus motif since it is not present in 
# the ATtRACT database.
pathToMotifs <- system.file("extdata", "motifs.txt", package="circRNAprofiler")
...
- foregroundNorm: number of motif found in the foreground target sequences (+1) 
...
non-canonical matches), continuous canonical WC matches and non-canonical matches
between the 2 given sequences (miRNA seed region and  miRNA seed region binding
sites) are retrieved. A WC match occurs when adenosine (A) pairs with uracil (U) 
and guanine (G) pairs with cytosine (C). A perfect seed match has no gaps in 
alignment within the WC matching. A non-canonical match occurs when a G matches
with U. A mismatch occurs when there is a gap in one of the two sequences 
(asymmetric mismatch) or in the following cases (symmetric mismatch): 
A-G or A-C or U-C. Only miRNA seed region binding sites that meet specific 
matching requirements are collected and reported in the final
output. Of those binding sites, additional features are also reported. These 
features are the number of matches found between the target sequences and the 
...
```{r,eval = FALSE}
# Screen circRNA sequences for miRNA binding sites
...
# miRs.txt (optional). 
# For example purpose we put "miRs.txt." in "extdata" so that it can be used 
# with this vignettes.  We only reported 3 miRs in miRs.txt 
...
# different sheet of an xlsx file for a better user consultaion.
i <- 1
j <- 1
...
If compementary = TRUE, only back-spliced junctions of circRNAs of which flanking 
