---
title: 'circRNAprofiler: An R-based computational framework for downstream analysis of circRNA molecules'
...
and other. The following project files are also needed: experiment.txt, motifs.txt, 
...
If the tools is not among the possible options, first check that the tool used 
is an annotation-based circRNA detection tool, rename the file to circRNAs_X.txt
and put them in the subfolder "other".  In this last case, you must ensure that 
...
detection tool and the occurrences found in each sample. 
...
NOTE:
CircRNA that derived from the antisense strand are removed from the data frame 
and stored in a file (antisenseCircRNAs.txt) for user consultation. 
A circRNA is defined antisense if the strand reported in the prediction results 
is different from the strand reported in the genome annotation file. 


Modules in circRNAprofiler are specific for the analysis of circRNAs that derive
from the sense strand of the gene.
...
file longer than the predicted length. 
...
iii) sequences of the back-spliced exons. In detail, if type = "ie" the sequences are 
retrieved from the genomic window defined by using the values given in input 
to lIntron and lExon. If type = "bse" the sequences of the back-spliced exons 
will be extracted. If type = "fi" the sequences of the introns flanking the 
back-spliced exons are retrieved.
...
nucleotides. If the identified motifs matches with or they are part 
...
- backgroundNorm: number of motifs found in background target sequences and
normalized for the number of sequences analyzed.
...
non-canonical matches between the 2 given sequences (miRNA seed region and miRNA 
miRNA seed region binding sites) are retrieved. A WC match occurs when 
adenosine (A) pairs with uracil (U) and guanine (G) pairs with cytosine (C). 
A perfect seed match has no gaps in alignment within the WC matching. A 
non-canonical match occurs when a G matches with U. A mismatch occurs when there
is a gap in one of the two sequences (asymmetric mismatch) or in the following 
cases (symmetric mismatch): A-G or A-C or U-C. Only miRNA seed region binding sites
that meet specific matching requirements are collected and reported in the final
...
- counts: is the number of times a miRNA seed region binding sites is found in 
the target sequence.

- totMatchesInSeed: is the number of total matches (canonical WC + non-canonical matches)
found between the miRNA seed region and the miRNA seed region binding sites in 
the target sequence.

- cwcMatchesInSeed: is the number of continuous WC matches found between the miRNA
seed region and the miRNA seed region binding sites in the target sequence.

- seedLocation: is the location of the miRNA seed region binding sites in the 
...
region binding sites in the target sequence (e.g. position 16 to 19).

- cwcMatchesInCentral: is the number of continuous WC matches found between the 
miRNA central region and the sequence upstream the miRNA seed region binding sites
...
compensatory region and the sequence upstream the miRNA seed region binding sites
...
binding sites in the target sequence (e.g. position 12 to 15).

- localAUcontent: is the percentage of AU nucleotides in the 10 nucleotides upstream
and downstream the miRNA seed region binding sites in the target sequence.
...
that there are 7 matches between the miRNA seed region binding sites in the target 
...
binding sites. The 30 nucleotides skipped in the initial phase are analyzed in 
