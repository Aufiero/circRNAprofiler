...
hypertrophic cardiomyopathies (HCM). After the detections we ran through
Modules 1-3 to generate the object backSplicedJunctions and mergedBSJunctions
...
```{r, echo=FALSE, out.width='45%', fig.align="center", fig.cap="\\label{fig:figs} Example of a project folder structure"}
...
It must have 3 columns with header:
...
the ATtRACT or MEME database and custom motifs reported in motifs.txt are reversed and 
analyzed together with the direct motifs as they are reported in the databases 
...
It might be that the above URL of the databases from which the RBP motifs are 
retrierved for some reason do not work, in that case a message will be printed 
...
using the number or the total length of the target sequences.
...
# 
# Normalize using circRNA sequences:
# nf1 = sum(annotatedCirc$lenCircRNA, na.rm = T) 
# nf2 = sum(annotatedBackgroundCircs$lenCircRNA, na.rm = T)

# Normalize using the BSJ sequences:
# nf1 = 22* nrow(annotatedCirc) 
# nf2 = 22* nrow(annotatedBackgroundCircs)
