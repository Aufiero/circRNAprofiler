...
test_that("getSeqFromGRs() retrieves the correct genomic ranges when
          type = ie",
          {
              gtf <- formatGTF(pathToGTF = "gencodeVM16.gtf")
              # Create the backSplicedJunctions without retriving the missing coordinates
              ...
              expect_equal(substr(targets$upGR$seq[4], 1, 20), "AUUUAACUUCAGAGAGUCAC")
              expect_equal(substr(targets$downGR$seq[4], 1, 20),
                           "AAUUACUGAGGUAAAUAGAG")

              expect_equal(substr(targets$upGR$seq[3], 1, 20), "UUAACCACCACAUAGCCUAU")
...
test_that("getSeqsFromGRs() extracts BSJ coordinates when type = bse", {
    ...
    # Check whether the right sequences are retrieved
    expect_equal(substr(targets$upGR$seq[4], 1, 20), "AUGGAGCAACUAUCCGAUGA")
    expect_equal(substr(targets$downGR$seq[4], 1, 20),
                 "AAGCUCUGAAUCAGGGUGAG")

    expect_equal(substr(targets$upGR$seq[3], 1, 20), "GUAACUAAAUGACCAUGGAA")
    expect_equal(substr(targets$downGR$seq[3], 1, 20),
...
test_that("getSeqsFromGRs() extracts the coordinates of the flanking introns
    ...
    # Check whether the right sequences are retrieved
    expect_equal(substr(targets$upGR$seq[4], 1, 20), "GUGAGUGCUCCGCCACCUGG")
    expect_equal(substr(targets$downGR$seq[4], 1, 20),
                 "GUAAAUAGAGAAAAUUUCUC")

    expect_equal(substr(targets$upGR$seq[3], 1, 20), "GUAAGAGGAGCAGGAGGAGG")
    expect_equal(substr(targets$downGR$seq[3], 1, 20),
